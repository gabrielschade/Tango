...
namespace Tango.Test.Types
{
...

    {
        ...
        {
            IEnumerable<(bool, int)> result =
                Collection.CountBy(_isEven, _10000values);

            int expected = 10000;
            ((bool, int), (bool, int)) headAndTailEnd = Collection.HeadAndTailEnd(result);
        ...
        public void CollectionZip()
        {
            ...
            Assert.IsTrue(
                Collection.ForAll2( 
                    (element1, element2) => element1.Item1 == element2, pairs, _0to10values)
                && Collection.ForAll2(
                    (element1, element2) => element1.Item2 == element2, pairs, _10to0values)
                    );
        }
